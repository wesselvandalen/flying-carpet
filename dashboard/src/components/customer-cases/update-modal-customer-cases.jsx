...
export default function UpdateModalCustomerCases({ customerCase, handleShowModal, showModal, handleAlertShow }) {
    ...
    const [description, setDescription] = useState(customerCase.description);
    const [image, setImage] = useState(customerCase.image);
    const [sector, setSector] = useState(customerCase.sector);
    const [clientName, setClientName] = useState(customerCase.clientName);

    const handleUpdate = async (e) => {
        e.preventDefault();
        const object = { "id": customerCase.id, title, description, image, sector, clientName };

        if (title === undefined || title.trim() === "") {
            object.title = customerCase.title;
        }
        if (description === undefined) {
            object.description = customerCase.description;
        }
        if (image === undefined) {
            object.image = customerCase.image;
        }
        if (sector === undefined) {
            object.sector = customerCase.sector;
        }
        if (clientName === undefined) {
            object.clientName = customerCase.clientName;
        }

        await updateCustomerCase(object);
    ...
    const handleDelete = async (e) => {
        e.preventDefault();
        await deleteCustomerCaseById(customerCase.id);
        window.location.reload();
    }
                ...
                <div className="relative p-4 bg-white rounded-lg shadow sm:p-5">
                    <div className="flex justify-between items-center pb-4 mb-4 rounded-t border-b sm:mb-5">
                        <h3 className="text-lg font-semibold text-gray-900">
                            Update {handleLongNames(customerCase.title)}
                        </h3>
                    ...
                    <form action="#">
                        <div className="grid gap-4 mb-4 sm:grid-cols-2">
                            <div>
                                ...
                                <input onChange={(e) => setTitle(e.target.value)} type="text" name="title" id="title" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5" placeholder={customerCase.title} required="" />
                            </div>
                            <div>
                                <label htmlFor="description" className="block mb-2 text-sm font-medium text-gray-900">Description</label>
                                <input onChange={(e) => setDescription(e.target.value)} type="text" name="description" id="description" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5" placeholder={customerCase.description} required="" />
                            </div>
                            <div>
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            >
                                ...
                                Remove {handleLongNames(customerCase.title)}
                            </button>
                        </div>
                    </form>
                </div>
