...
export default function UpdateModalInternNetwork({ network, handleShowModal, showModal }) {
    ...
    const handleUpdate = async (e) => {
        ...
        if (description === undefined) {
            object.description = network.description;
        }
        if (image === undefined) {
            object.image = network.image;
        }
        if (focusArea === undefined) {
            object.focusArea = network.focusArea;
        }

        await updateInternNetwork(object);
    ...
    const handleDelete = async (e) => {
        e.preventDefault();
        await deleteInternNetworkById(network.id);
        window.location.reload();
    }
                ...
                <div className="relative p-4 bg-white rounded-lg shadow sm:p-5">
                    <div className="flex justify-between items-center pb-4 mb-4 rounded-t border-b sm:mb-5">
                        <h3 className="text-lg font-semibold text-gray-900">
                            Update {handleLongNames(network.name)}
                        </h3>
                    ...
                    <form action="#">
                        <div className="grid gap-4 mb-4 sm:grid-cols-2">
                            <div>
                                ...
                                <input onChange={(e) => setName(e.target.value)} type="text" name="name" id="name" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5" placeholder={network.name} />
                            </div>
                            <div>
                                <label htmlFor="description" className="block mb-2 text-sm font-medium text-gray-900">Description</label>
                                <input onChange={(e) => setDescription(e.target.value)} type="text" name="description" id="description" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5" placeholder={network.description} />
                            </div>
                            <div>
                                <label htmlFor="image" className="block mb-2 text-sm font-medium text-gray-900">Image</label>
                                <input onChange={(e) => setImage(e.target.value)} type="text" name="image" id="image" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5" placeholder={network.image} />
                            </div>
                            <div>
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            >
                                ...
                                Delete {handleLongNames(network.name)}
                            </button>
                        </div>
                    </form>
                </div>
